/// 2MR (2-Mile Run) scoring tables with per-age and per-sex columns
/// and a fill-down rule, analogous to SDC implementation.
///
/// Source CSV format (20 columns):
/// Points, M17–21, F17–21, M22–26, F22–26, ..., M62+, F62+
///
/// Values are times in mm:ss. Faster (smaller) times earn higher points.
/// Awarding rule: scan from 100 down; return the first (highest) point whose
/// requirement is met i.e., actual time <= requirement time for that point.
///
...
///
/// Exposes:
/// - void loadRun2miCsv(String csv)
/// - int run2miPointsForSex(AftSex sex, int age, Duration time)
library run2mi_table;
...
/// Raw tables (sparse: only provided points will exist). Store times in seconds.
Map<int, List<int>> _rawMale = <int, List<int>>{};
Map<int, List<int>> _rawFemale = <int, List<int>>{};

/// Dense tables for each sex: [P=100..0][ageIdx=0..9] -> time requirement (seconds, no -1 remain).
...
/// Values are mm:ss or -1; they are converted to seconds (int).
...
void loadRun2miCsv(String csv) {
  ...
  for (final rawLine in lines) {
    ...
    for (var ageIdx = 0; ageIdx < _numAges; ageIdx++) {
      ...
      final fIdx = mIdx + 1;
      maleRow[ageIdx] = _toSeconds(parts[mIdx]);
      femaleRow[ageIdx] = _toSeconds(parts[fIdx]);
    }
    male[p] = maleRow;
    female[p] = femaleRow;
  }
...
/// Compute 2MR points (0–100) for given sex, age and run time.
/// Returns the highest (topmost) point P where actual time <= requirement time.
int run2miPointsForSex(AftSex sex, int age, Duration time) {
  _ensureBuilt();
